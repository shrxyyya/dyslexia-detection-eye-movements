...
def generate_synthetic_fixations_corrected(original_fixations, subject_id, perturbation_factor=0.05):
    ...
    for i in range(len(synthetic_fix)):
        ...
        synthetic_fix.iloc[i, synthetic_fix.columns.get_loc('end_ms')] = cumulative_time + synthetic_fix.iloc[i]['duration_ms']
...
# Load original data
fixations = pd.read_csv('real-data/Subject_1996_T4_Meaningful_Text_fixations.csv')

# Generate corrected synthetic data
print("Generating corrected synthetic data with fix_y unchanged...")
synthetic_fix_2996 = generate_synthetic_fixations_corrected(fixations, 2996, perturbation_factor=0.03)
synthetic_fix_3996 = generate_synthetic_fixations_corrected(fixations, 3996, perturbation_factor=0.05)

# Save corrected data
synthetic_fix_2996.to_csv('synthetic-data/Subject_2996_T4_Meaningful_Text_fixations.csv', index=False)
synthetic_fix_3996.to_csv('synthetic-data/Subject_3996_T4_Meaningful_Text_fixations.csv', index=False)
